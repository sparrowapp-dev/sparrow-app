<script lang="ts">
  ...
  const navigateToDirectory = (elem) => {
    let response = searchTreeDocument(collection, elem._id ? elem._id : elem.id);
...
<div class="save-request-backdrop {visibility ? 'd-block' : 'd-none'}" on:click={()=>{onClick(false)}}>
</div>
<div class="save-request {visibility ? 'd-block' : 'd-none'}">
    <div class="contain">
        <div class="d-flex justify-content-between">
            ...
            <button class="btn" on:click={()=>{onClick(false)}}><img src={crossAsset} alt=""></button>
        </div>
        <div class="url d-flex align-items-center pb-3" style="height:20px;">
                <!-- svelte-ignore a11y-click-events-have-key-events -->
                <!-- svelte-ignore a11y-no-static-element-interactions -->
                {#if path.length > 0}
                    <span class="d-flex align-items-center justify-content-center cursor-pointer" style="height:24px; width:24px;" on:click={navigateToLastRoute}><img src={leftArrowAsset} alt=""></span>
                {/if}
                {#if workspace}
                    <span on:click={navigateToWorkspace} class="cursor-pointer px-1"  style="font-size: 12px;"> <img style="height:10.67px; width: 10.67px;" src={workspaceAsset} alt=""> {workspace.name}</span>
                {/if}
                {#if path.length > 0}
                {#each path as elem}
                <span>/</span>
                ...
                }} class="cursor-pointer px-1" style="font-size: 12px;">
                    {#if elem.type === ItemType.COLLECTION}
                        <img src={collectionAsset} style="height:10.67px; width: 10.67px;" alt="">
                    {:else if elem.type === ItemType.FOLDER}
                    <img src={folderAsset} style="height:10.67px; width: 10.67px;" alt="">
                    {/if}
                {elem.name}</span> 
                {/each}
                {/if}
        ...
        <div class="row">
            <div class="col-6" style="border-right: 1px solid var(--border-color);">
                <div style="height: 460px; overflow:auto;">
                    ...
                    <p class="mb-0"><small class="save-text-clr">Collection: </small> <small> {path[path.length-1].name}</small> </p>
                    <small class="save-text-clr" style="font-size: 12px;">Save your request in this collection or any of its folders.</small>
                {:else if path.length > 0 && path[path.length - 1].type === ItemType.FOLDER}
                    <p class="mb-0"><small class="save-text-clr">Folder: </small> <small> {path[path.length-1].name}</small> </p>
                    {:else}
                    <p class="mb-0"><small class="save-text-clr">Workspace: </small> 
                        {#if workspace}
                            <small>
                                {workspace.name}
                            </small> 
                    ...
                    <small class="save-text-clr" style="font-size: 12px;">Save your request inside a collection or a folder.</small>
                {/if}
                <p></p>
            ...
            <div class="col-6">
                ...
                <div class="pb-1">
                    <input type="text" style="width: 100%; font-size: 12px;" placeholder="Request Name" class="p-1 bg-black outline-0 rounded border-0">
                </div>
                <p class="save-text-clr mb-1" style="font-size:12px">Description</p>
                <div class="pb-1">
                    <textarea style="width: 100%; font-size: 12px;" class="p-1 bg-black rounded border-0" rows="3" placeholder="Give a description to help people know about this request."></textarea>
                </div>
                <p class="save-text-clr mb-1" style="font-size:12px">Saving to</p>
                {#if path.length === 0}
                    <p style="font-size: 12px;" class="save-text-clr">Select a Collection or Folder.</p>
                {:else} 
                <div class="url d-flex align-items-center pb-3">
                    <!-- svelte-ignore a11y-click-events-have-key-events -->
                    <!-- svelte-ignore a11y-no-static-element-interactions -->
                    {#if workspace}
                        <span class="px-1"  style="font-size: 12px;"> <img style="height:10.67px; width: 10.67px;" src={workspaceAsset} alt=""> {workspace.name}</span>
                    {/if}
                    {#if path.length > 0}
                    {#each path as elem}
                    <span>/</span>
        ...
        <div class="controllers mt-3 d-flex justify-content-between">
            <div>
                {#if path.length === 0}
                    <IconButton text = {"+ Collection"} onClick= {()=>{}} />
                {:else if path.length > 0 && path[path.length - 1].type === ItemType.COLLECTION}
                    <IconButton text = {"+ Folder"} onClick= {()=>{}} />
                {/if}
            </div>
            <div>
                <span class="mx-2">
                    <CoverButton text = {"Cancel"} size={16} type={"dark"} onClick= {()=>{onClick(false)}} />
                </span>
                <CoverButton text = {"Save"} size={16} type={"primary"} onClick= {()=>{
                    handleSaveRequest()
                }} />
            </div>
        </div>
