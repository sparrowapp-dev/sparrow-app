  ...
  const fetchComponentData = (id : string, list : NewTab[]) => {
    list.forEach((elem : NewTab) => {
      if (elem.id === id) {
        ...
        headersCount =  requestData.request.autoGeneratedHeaders.length + requestData.request.headers.length - 1;
        parametersCount =  requestData.request.queryParams.length - 1;
        
        if(findAuthHeader(requestData).key) {
          headersCount += 1;
        }
        
        if(findAuthParameter(requestData).key) {
          parametersCount += 1;
        }
    ...
    >
      <span
        ...
        ><span
          ...
          >Parameters
          <p style="font-size: 12px;" class="mb-0 text-labelColor ps-1">({parametersCount})</p>
        </span>
      </span>
      ...
      <span
        ...
        ><span
          ...
          >Headers
          <p style="font-size: 12px;" class="mb-0 text-labelColor ps-1">({headersCount})</p>
        </span>
      </span>
