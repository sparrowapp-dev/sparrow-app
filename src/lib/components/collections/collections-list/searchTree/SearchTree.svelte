...
<div>
  ...
  <div>
    <div style="height:36px;" class="d-flex align-items-center" on:click={() => {folderExpand = !folderExpand;}}>
    ...
    <div style="padding-left: 15px; cursor:pointer; display: {folderExpand ? 'block' : 'none'};">
...
{:else if explorer.type === "REQUEST"}
  <div style="padding-left: 0; cursor:pointer;">
    <Request path={path} name={explorer.name} method={explorer.request.method} />
  </div>
{:else}
