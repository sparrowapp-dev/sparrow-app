...
pub async fn import_swagger_url(url: &str, headers: &str, workspaceid: &str) -> Result<Value, Box<dyn std::error::Error>> { 
    ...
    let posturl = format!("{}/api/workspace/{}/importJson/collection", config.api_url, workspaceid);
    ...
    let headers_json: serde_json::Value = response_headers.iter().map(|(name, value)| {
