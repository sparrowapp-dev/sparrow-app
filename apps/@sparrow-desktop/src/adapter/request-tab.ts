...
export class RequestTabAdapter {
  ...
  public adapt(
    ...
    adaptedRequest.updateTests(request.request?.tests);
    adaptedRequest.updateAutoGeneratedHeaders(
    ...
    if (selectedRequestAuthType) {
      const AuthType = this.setAuthType(
        request.request?.selectedRequestAuthType,
      );
      adaptedRequest.updateState({
    ...
    const selectedRequestAuthProfileId =
      request.request?.selectedRequestAuthProfileId;
    ...
    if (body) {
      ...
      const formdata = [...fileData, ...textData];
  ...
  public unadapt(requestTab: Tab) {
    ...
    return {
      ...
      tests: requestTab.property.request?.tests,
      ...
      selectedRequestAuthType: this.unsetAuthType(
        requestTab.property.request?.state
          ?.requestAuthNavigation as HttpRequestAuthTypeBaseEnum,
      ),
      selectedRequestAuthProfileId:
        requestTab.property.request?.state?.selectedRequestAuthProfileId,
    };
  }
}
