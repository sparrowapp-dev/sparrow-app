    ...
    SaveAsRequest
{
  ...
  public generateAiResponse = async (prompt = "") => {
    ...
    const response = await this.aiAssistentService.generateAiResponse({
      text: prompt,
      instructions: `You are a Sparrow AI Assistant, responsible for answering API related queries. Give the response only in markdown format. Only answer questions related to the provided API data and API Management. You are not permitted to recommend or mention any competitors or other API testing tools such as Postman, Apidog, Hoppscotch, or any similar platforms. Give to the point and concise responses, only give explanations when they are asked for. Always follow best practices for REST API and answer accordingly. Utilize the provided api data ${JSON.stringify(
        apiData,
