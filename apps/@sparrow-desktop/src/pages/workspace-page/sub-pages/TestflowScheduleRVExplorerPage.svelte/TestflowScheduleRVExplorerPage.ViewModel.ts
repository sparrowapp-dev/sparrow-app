...
export class TestflowScheduleRVExplorerPageViewModel {
  ...
  private updateNodesDebounce = async (_nodes: TFNodeType[]) => {
    ...
    progressiveTab.property.testflowScheduleRunView.nodes = nodes;
  ...
  private updateEdgesDebounce = async (_edges: TFEdgeType[]) => {
    ...
    progressiveTab.property.testflowScheduleRunView.edges = edges;
    this.tab = progressiveTab;
    this.tabRepository.updateTab(progressiveTab.tabId, progressiveTab);
    this.compareTestflowWithServer();
  };
  ...
  private compareTestflowWithServerDebounced = async () => {
    ...
    // edges
      ...
      !this.compareArray.init(
        ...
        progressiveTab.property.testflowScheduleRunView.edges,
      )
    ) {
      result = false;
    }
  ...
  public handleTestFlowRun = async (_id: string, _event: string) => {
    ...
    const nodes = progressiveTab?.property?.testflowScheduleRunView?.nodes;
    const edges = progressiveTab?.property?.testflowScheduleRunView?.edges;
  ...
  public handleSingleTestFlowNodeRun = async (nodeId: string) => {
    ...
    const node = tab?.property?.testflowScheduleRunView?.nodes.find(
      (item) => item.id === nodeId,
    );
