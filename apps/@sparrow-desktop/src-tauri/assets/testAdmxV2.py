...
def create_adml_file(app_name, registry_entries, output_file):
    ...
    # Add presentation for each policy that needs it
    for entry in registry_entries:
        key_path = entry['key']
        ...
        for value in entry['values']:
            ...
            # Only create presentations for policies with elements
            if value_type != 'integer' or (value['value'] != '0' and value['value'] != '1'):
                policy_name = generate_policy_name(key_path, value_name)
                ...
                if value_type == 'string':
                    ET.SubElement(presentation, 'textBox', {
                        'refId': policy_name
                    })
                    ET.SubElement(presentation, 'label').text = f"Enter value for {value_name if value_name else 'default value'}"
                elif value_type == 'integer' or value_type == 'dword':
                    ET.SubElement(presentation, 'decimalTextBox', {
                        'refId': policy_name
                    })
                    ET.SubElement(presentation, 'label').text = f"Enter numeric value for {value_name}"
