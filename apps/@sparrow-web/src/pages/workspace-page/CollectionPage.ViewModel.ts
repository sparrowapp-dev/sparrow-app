...
import { type EventsValues, type State as SocketIOStateType } from "@sparrow/common/types/workspace/socket-io-request-tab";
import type { GraphqlRequestAuthTabInterface, GraphqlRequestAutoGeneratedHeadersTabInterface, GraphqlRequestStateTabInterface } from "@sparrow/common/types/workspace/graphql-request-tab";
...
export default class CollectionsViewModel {
  ...
  private createRESTDuplciateTab = async (tabId: string) => {
    ...
    const currWorkSpace = await this.workspaceRepository.getActiveWorkspaceDoc();
    console.log("curr workspace :>> ", currWorkSpace)
    console.log("original Request Tab :>>", originalTab)
    if (currWorkSpace) {
      const newRequestTab = new InitRequestTab(
        "UNTRACKED-" + uuidv4(),
        currWorkSpace._id,
      );
      ...
      if (originalTab._data) {
        const { id, tabId, timestamp, ...restOfData } = originalTab._data;

        if (restOfData.name) newRequestTab.updateName(restOfData.name);
        if (restOfData.description) newRequestTab.updateDescription(restOfData.description);
        // if (restOfData.path) newRequestTab.updatePath(restOfData.path as TabPath);
        if (restOfData.property?.request?.url) newRequestTab.updateUrl(restOfData.property.request.url);
        if (restOfData.property?.request?.method) newRequestTab.updateMethod(restOfData.property.request.method as RequestMethodEnum);
        if (restOfData.property?.request?.body) newRequestTab.updateBody(restOfData.property.request.body as Body);
        if (restOfData.property?.request?.headers) newRequestTab.updateHeaders(restOfData.property.request.headers as KeyValueChecked[]);
        if (restOfData.property?.request?.queryParams) newRequestTab.updateQueryParams(restOfData.property.request.queryParams as KeyValueChecked[]);
        if (restOfData.property?.request?.auth) newRequestTab.updateAuth(restOfData.property.request.auth as Auth);
        if (restOfData.property?.request?.autoGeneratedHeaders) newRequestTab.updateAutoGeneratedHeaders(restOfData.property.request.autoGeneratedHeaders as KeyValueChecked[]);
        if (restOfData.property?.request?.state) newRequestTab.updateState(restOfData.property.request.state as StatePartial);
  ...
  private createWebSocketDuplciateTab = async (tabId: string) => {
    const originalTab = await this.tabRepository.getTabById(tabId) // Tab to be copied
    const currWorkSpace = await this.workspaceRepository.getActiveWorkspaceDoc();
    console.log("original WebSoc Tab :>>", originalTab)

    if (currWorkSpace) {
      const newWebSocketTab = this.initTab.webSocket("UNTRACKED-" + uuidv4(), currWorkSpace._id)
      console.log("new req ;>> ", newWebSocketTab)
      MixpanelEvent(Events.ADD_NEW_API_REQUEST, { source: "TabBar" });
      ...
      if (originalTab._data) {
        ...
        if (restOfData.name) newWebSocketTab.updateName(restOfData.name);
        if (restOfData.description) newWebSocketTab.updateDescription(restOfData.description);
        // if (restOfData.path) newWebSocketTab.updatePath(restOfData.path as TabPath);
        if (restOfData.property?.websocket?.url) newWebSocketTab.updateUrl(restOfData.property.websocket.url);
        if (restOfData.property?.websocket?.headers) newWebSocketTab.updateHeaders(restOfData.property.websocket.headers as KeyValueChecked[]);
        if (restOfData.property?.websocket?.queryParams) newWebSocketTab.updateQueryParams(restOfData.property.websocket.queryParams as KeyValueChecked[]);
        if (restOfData.property?.websocket?.message) newWebSocketTab.updateMessage(restOfData.property.websocket.message);
        if (restOfData.property?.websocket?.autoGeneratedHeaders) newWebSocketTab.updateAutoGeneratedHeaders(restOfData.property.websocket.autoGeneratedHeaders as KeyValueChecked[]);
        if (restOfData.property?.websocket?.state) newWebSocketTab.updateState(restOfData.property.websocket.state as Partial<WebSocketStateType>);
  ...
  private createSocketIODuplciateTab = async (tabId: string) => {
    const originalTab = await this.tabRepository.getTabById(tabId) // Tab to be copied
    const currWorkSpace = await this.workspaceRepository.getActiveWorkspaceDoc();
    console.log("original SocIO Tab :>>", originalTab)

    if (currWorkSpace) {
      const newSocketIOTab = this.initTab.socketIo("UNTRACKED-" + uuidv4(), currWorkSpace._id)
      console.log("new req ;>> ", newSocketIOTab)
      MixpanelEvent(Events.ADD_NEW_API_REQUEST, { source: "TabBar" });
      ...
      if (originalTab._data) {
        ...
        if (restOfData.name) newSocketIOTab.updateName(restOfData.name);
        if (restOfData.description) newSocketIOTab.updateDescription(restOfData.description);
        // if (restOfData.path) newSocketIOTab.updatePath(restOfData.path as TabPath);
        if (restOfData.property?.socketio?.url) newSocketIOTab.updateUrl(restOfData.property.socketio.url);
        if (restOfData.property?.socketio?.headers) newSocketIOTab.updateHeaders(restOfData.property.socketio.headers as KeyValueChecked[]);
        if (restOfData.property?.socketio?.queryParams) newSocketIOTab.updateQueryParams(restOfData.property.socketio.queryParams as KeyValueChecked[]);
        if (restOfData.property?.socketio?.events) newSocketIOTab.updateEvents(restOfData.property.socketio.events as EventsValues[]);
        if (restOfData.property?.socketio?.events) newSocketIOTab.updateEventName(restOfData.property.socketio.eventName);
        if (restOfData.property?.socketio?.message) newSocketIOTab.updateMessage(restOfData.property.socketio.message);
        if (restOfData.property?.socketio?.autoGeneratedHeaders) newSocketIOTab.updateAutoGeneratedHeaders(restOfData.property.socketio.autoGeneratedHeaders as KeyValueChecked[]);
        if (restOfData.property?.socketio?.state) newSocketIOTab.updateState(restOfData.property.socketio.state as Partial<SocketIOStateType>);
  ...
  private createGraphQLDuplciateTab = async (tabId: string) => {
    ...
    if (currWorkSpace) {
      const newGraphQLTab = this.initTab.graphQl("UNTRACKED-" + uuidv4(), currWorkSpace._id)
      console.log("new req ;>> ", newGraphQLTab)
      MixpanelEvent(Events.ADD_NEW_API_REQUEST, { source: "TabBar" });
      ...
      if (originalTab._data) {
        ...
        if (restOfData.name) newGraphQLTab.updateName(restOfData.name);
        if (restOfData.description) newGraphQLTab.updateDescription(restOfData.description);
        // if (restOfData.path) newGraphQLTab.updatePath(restOfData.path as TabPath);
        if (restOfData.property?.graphql?.url) newGraphQLTab.updateUrl(restOfData.property.graphql.url);
        if (restOfData.property?.graphql?.headers) newGraphQLTab.updateHeaders(restOfData.property.graphql.headers as KeyValueChecked[]);
        if (restOfData.property?.graphql?.mutation) newGraphQLTab.updateMutation(restOfData.property.graphql.mutation);
        if (restOfData.property?.graphql?.schema) newGraphQLTab.updateSchema(restOfData.property.graphql.schema);
        if (restOfData.property?.graphql?.variables) newGraphQLTab.updateVariables(restOfData.property.graphql.variables);
        if (restOfData.property?.graphql?.auth) newGraphQLTab.updateAuth(restOfData.property.graphql.auth as GraphqlRequestAuthTabInterface);
        if (restOfData.property?.graphql?.autoGeneratedHeaders) newGraphQLTab.updateAutoGeneratedHeaders(restOfData.property.graphql.autoGeneratedHeaders as GraphqlRequestAutoGeneratedHeadersTabInterface[]);
        if (restOfData.property?.graphql?.state) newGraphQLTab.updateState(restOfData.property.graphql.state as Partial<GraphqlRequestStateTabInterface>);
  ...
  public createDuplicateTabByTabId = async (tabId: string) =>{
    const originalTab = await this.tabRepository.getTabById(tabId) // Tab to be copied
    const originalTabType = originalTab._data.type as TabTypeEnum;
    const currWorkSpace = await this.workspaceRepository.getActiveWorkspaceDoc();
