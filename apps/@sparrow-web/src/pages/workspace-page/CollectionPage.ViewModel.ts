...
export default class CollectionsViewModel {
  ...
  private createRESTDuplciateTab = async (tabId: string) => {
    ...
    if (currWorkSpace) {
      ...
      const { id, tabId, timestamp, ...restOfData } = originalTab.toMutableJSON();

        newRequestTab.updateName(restOfData.name);
        newRequestTab.updateDescription(restOfData.description as string);
        newRequestTab.updatePath(restOfData.path as TabPath);
        newRequestTab.updateUrl(restOfData.property.request.url);
        newRequestTab.updateMethod(restOfData.property.request.method as RequestMethodEnum);
        newRequestTab.updateBody(restOfData.property.request.body as Body);
        newRequestTab.updateHeaders(restOfData.property.request.headers as KeyValueChecked[]);
        newRequestTab.updateQueryParams(restOfData.property.request.queryParams as KeyValueChecked[]);
        newRequestTab.updateAuth(restOfData.property.request.auth as Auth);
        newRequestTab.updateAutoGeneratedHeaders(restOfData.property.request.autoGeneratedHeaders as KeyValueChecked[]);
        newRequestTab.updateState(restOfData.property.request.state as StatePartial);
  ...
  private createWebSocketDuplciateTab = async (tabId: string) => {
    ...
    if (currWorkSpace) {
      ...
      newWebSocketTab.updateDescription(restOfData.description as string);
      newWebSocketTab.updatePath(restOfData.path as TabPath);
      newWebSocketTab.updateUrl(restOfData.property.websocket.url);
      newWebSocketTab.updateHeaders(restOfData.property.websocket.headers as KeyValueChecked[]);
      newWebSocketTab.updateQueryParams(restOfData.property.websocket.queryParams as KeyValueChecked[]);
      newWebSocketTab.updateMessage(restOfData.property.websocket.message);
      newWebSocketTab.updateAutoGeneratedHeaders(restOfData.property.websocket.autoGeneratedHeaders as KeyValueChecked[]);
      newWebSocketTab.updateState(restOfData.property.websocket.state as Partial<WebSocketStateType>);
  ...
  private createSocketIODuplciateTab = async (tabId: string) => {
    ...
    if (currWorkSpace) {
      ...
      newSocketIOTab.updatePath(restOfData.path as TabPath);
      newSocketIOTab.updateUrl(restOfData.property.socketio.url);
      newSocketIOTab.updateHeaders(restOfData.property.socketio.headers as KeyValueChecked[]);
      newSocketIOTab.updateQueryParams(restOfData.property.socketio.queryParams as KeyValueChecked[]);
      newSocketIOTab.updateEvents(restOfData.property.socketio.events as EventsValues[]);
      ...
      newSocketIOTab.updateAutoGeneratedHeaders(restOfData.property.socketio.autoGeneratedHeaders as KeyValueChecked[]);
      newSocketIOTab.updateState(restOfData.property.socketio.state as Partial<SocketIOStateType>);
  ...
  private createGraphQLDuplciateTab = async (tabId: string) => {
    ...
    if (currWorkSpace) {
      ...
      // newGraphQLTab.updatePath(restOfData.path as TabPath);
      newGraphQLTab.updateUrl(restOfData.property.graphql.url);
      newGraphQLTab.updateHeaders(restOfData.property.graphql.headers as KeyValueChecked[]);
      ...
      newGraphQLTab.updateAuth(restOfData.property.graphql.auth as GraphqlRequestAuthTabInterface);
      newGraphQLTab.updateAutoGeneratedHeaders(restOfData.property.graphql.autoGeneratedHeaders as GraphqlRequestAutoGeneratedHeadersTabInterface[]);
      newGraphQLTab.updateState(restOfData.property.graphql.state as Partial<GraphqlRequestStateTabInterface>);
