...
<div class="popup p-4" style="top: {top}px; left: {left}px;">
  ...
  <div class="close-icon" on:click={handleClose}>
    <CrossIcon height={"12px"} width={"12px"} color={"var(--icon-tertiary-100)"}/>
  </div>

  <!-- Title and content -->
  <h2 class="text-fs-16" style="font-weight: 500; color:var(--text-primary-300);">{title}</h2>
  <p class="text-fs-13"  style="line-height: 19.5px; font-weight:400;">{description}</p>

  <!-- Next/Done button -->
  <button class=" mt-2 px-2 py-1 rounded-1 text-fs-12 " style="font-weight:400; border: none; background-color:#2A2C3C;" on:click={handleNext}>Next</button>
</div>
