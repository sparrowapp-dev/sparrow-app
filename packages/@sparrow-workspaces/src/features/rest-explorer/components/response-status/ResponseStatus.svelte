<script lang="ts">
  ...
  import { ArchiveRegular, BoxIcon, ClockIcon, ClockRegular, DotIcon } from "@sparrow/library/icons";
...
>
  ...
  >
    <div class="d-flex align-items-center gap-2 ms-auto" style="justify-content: right;">
      ...
      <div class="d-flex align-items-center gap-3">
        ...
        <Tooltip title="Time" placement={"top-center"} zIndex={500}>
          ...
          >
            <span class="d-flex">
              <ClockRegular size="12px" color={checkIfRequestSucceed(response?.status)
                  ? "var(--text-ds-success-300)"
                  : "var(--request-delete)"}/>
            </span>
            <span
              class="text-fs-12 "
              style=" margin-left:6px;color:{checkIfRequestSucceed(response?.status)
                ? 'var(--text-ds-success-300)'
                : 'var(--request-delete)'};"
            >
              {response.time}
