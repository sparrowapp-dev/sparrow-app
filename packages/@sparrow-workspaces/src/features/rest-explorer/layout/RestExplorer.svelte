  ...
  const handleGenerateMockData = async () => {
    ...
    if (
      ...
      if (Array.isArray(response.result) && response.result.length > 0) {
        isMergeViewEnableForHeaders = true;
        newModifiedContent = response.result;
      } else {
        isMergeViewLoading = false;
        return;
      }
      ...
      if (Array.isArray(response.result) && response.result.length > 0) {
        isMergeViewEnableForParams = true;
        newModifiedContent = response.result;
      } else {
        isMergeViewLoading = false;
        return;
      ...
      if (
        ...
        $tab.property?.request?.state?.requestBodyLanguage ===
        ...
        if (typeof jsonResult === "object" && jsonResult !== null) {
          newModifiedContent = JSON.stringify(response.result);
        } else {
          isMergeViewLoading = false;
          return;
        }
