<script lang="ts">
  ...
  const addBlockInBetween = (_id: string, _event: any) => {
      ...
      return [
        ...(_nodes || []),
        {
          ...
          data: {
            ...
            tabId: $tab.tabId,
            requestData: createBlankRequestObject("", "", ""),
          },
    ...
    if (_event === "add-block-after") {
      ...
      // debugger;
    } else if (_event === "add-block-before") {
      let destinationId;
      edges.update((edges) => {
        for (let i = 0; i < edges.length; i++) {
