<script lang="ts">
  ...
  const createBlankObject = (_url: string, _method: string, _name: string) => {
    const response: any = {};
  ...
  const updateSelectedAPI = async (
    ...
    let response: any = {};
    if (collectionId) {
      response = await createCustomObject(
        collectionId,
        requestId,
        folderId as string,
      );
      ...
      response = await createBlankObject(url as string, method, name);
    }
  ...
  const handleReplaceRequest = async () => {
    try {
      ...
      // populate existing api for the second time from the api dropdown
      await updateSelectedAPI(
        updateNodeId,
        updateNodeName,
        updateNodeRequestId,
   ...
   **/
  const handleReplaceRequestModal = async (
    tabId: string,
    nodeId: string,
    name: string,
    requestId: string,
  ...
  const createNewNode = async (_id: string, _requestData = undefined) => {
    ...
    let requestData;
    if (_requestData) {
      requestData = await createCustomObject(
...
>
  <SaveNode
    ...
    onSaveRequest={handleReplaceRequest}
